...
with open('mobile/lib/telas/painel_admin.dart', 'r', encoding='utf8') as f:
    content = f.read()

# 1. Add BlueThermalPrinter import
if 'blue_thermal_printer' not in content:
...
if '_pendingReleaseBatches' not in content:
    ...
    load_clients_re = re.compile(r'final clients = await api\.getAllClients\(\);')
    content = load_clients_re.sub("final clients = await api.getAllClients();\n      final pendingBatches = await api.getPendingBookBatches();\n      setState(() => _pendingReleaseBatches = pendingBatches);", content)

# 3. Add print method and showReceiveReturnDialog method
if '_printUnidadeBluetooth' not in content:
    methods = """
...
if 'Lotes Aguardando Liberação' not in content:
    pending_ui = """
                  ...
                  children: [
                    ...
                    ..._pendingReleaseBatches.map((batch) {
                      return ListTile(
                        ...
                        title: Text('Lote Fotógrafo: ${batch['photographer']['name']}', style: const TextStyle(color: Colors.white)),
...
if 'Receber Devolução de Book' not in content:
    return_card_ui = """
            GestureDetector(
...
"""
    content = re.sub(r'(const Text\(\'Toque para\\nver detalhes\',[^)]+\),\s*\n\s*\],\n\s*\),\n\s*\),)', r'\1' + return_card_ui, content)


# 6. Add Print Button to Book Tiles
content = content.replace("trailing: isRebolo", "trailing: Row(mainAxisSize: MainAxisSize.min, children: [IconButton(icon: const Icon(Icons.print, color: Colors.blueAccent), onPressed: () => _printUnidadeBluetooth(book), tooltip: 'Imprimir Ticket'), isRebolo")
content = content.replace("? const Icon(Icons.autorenew_rounded, color: Colors.orangeAccent)", "? const Icon(Icons.autorenew_rounded, color: Colors.orangeAccent)])")
content = content.replace(": const Icon(Icons.menu_book_rounded, color: Color(0xFFCE93D8)),", ": const Icon(Icons.menu_book_rounded, color: Color(0xFFCE93D8))]),")

with open('mobile/lib/telas/painel_admin.dart', 'w', encoding='utf8') as f:
    f.write(content)
