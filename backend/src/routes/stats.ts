...
router.get('/books', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!companyId) return res.status(403).json({ error: 'Sem empresa associada' });
    ...
    let analiseChildrens: any = { totalChildrenInSales: 0, idadeRanking: [], topNomes: [], faixaEtariaLucrativa: null };
    try {
      ...
      children.forEach(c => {
        if (c.age != null) ageMap[c.age] = (ageMap[c.age] ?? 0) + 1;
        if (c.name) {
    ...
    try {
      ...
      const eventAgg: Record<string, { books: number; faturamento: number }> = {};
      salesWithEvent.forEach(s => {
        const ev = eventClientMap[s.clientId] ?? 'Sem Evento';
        if (!eventAgg[ev]) eventAgg[ev] = { books: 0, faturamento: 0 };
        eventAgg[ev].books++;
    ...
    let analiseValores: any = { ticketMedio: 0, valorMaximo: 0, valorMinimo: 0, faturamentoTotal: 0, totalVendas: 0 };
    try {
    ...
    let analiseHorarios: any = { porHora: [], porDiaSemana: [], porMes: [], porSemanaDoMes: [], melhorDia: 'N/A', melhorHora: 0 };
    try {
      ...
      const weekOfMonthAgg: Record<number, number> = {};
    ...
    try {
      ...
      const photogValueAgg: Record<string, { count: number; sum: number }> = {};
      photogSales.forEach(s => {
        const pid = s.client?.photographerId;
        if (!pid) return;
        if (!photogValueAgg[pid]) photogValueAgg[pid] = { count: 0, sum: 0 };
      ...
      const allClientPhotogMap: Record<string, number> = {};
...
router.post('/books/ai-insights', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.user?.companyId;
    if (!companyId) return res.status(403).json({ error: 'Sem empresa associada' });
...
Retorne SOMENTE um array JSON: [{"emoji": "emoji aqui", "insight": "Texto do insight aqui."}]`;
    ...
    let insights: any[] = [];
    try { insights = JSON.parse(cleanJson); } catch { insights = [{ emoji: '🤖', insight: 'Tente novamente.' }]; }
    return res.json({ insights });
  } catch (error: any) {
    console.error('AI insights error:', error);
    if (error?.status === 429) return res.status(429).json({ error: 'Limite de requisicoes da IA atingido.' });
...
router.get('/rebolos', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    try {
      ...
      const reboloSoldByCity: Record<string, number> = {};
    ...
    let rankingVendedoresRecuperacao: any[] = [];
    try {
      const sellerRecoveryCount: Record<string, number> = {};
      rebolosSold.forEach(c => { if (c.assignedSellerId) sellerRecoveryCount[c.assignedSellerId] = (sellerRecoveryCount[c.assignedSellerId] ?? 0) + 1; });
    ...
    const bairroRecompraCount: Record<string, number> = {};
