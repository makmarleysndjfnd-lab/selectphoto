...
router.get('/pending', authMiddleware_1.authenticateToken, authMiddleware_1.requireAdminOrSupervisor, async (req, res) => {
    try {
        const userCompanyId = req.user?.companyId;
...
router.post('/:id/approve', authMiddleware_1.authenticateToken, authMiddleware_1.requireAdminOrSupervisor, async (req, res) => {
...
router.post('/:id/reject', authMiddleware_1.authenticateToken, authMiddleware_1.requireAdminOrSupervisor, async (req, res) => {
    const id = req.params.id;
    const userCompanyId = req.user?.companyId;
    try {
        const editRequest = await prisma.clientEditRequest.findFirst({
